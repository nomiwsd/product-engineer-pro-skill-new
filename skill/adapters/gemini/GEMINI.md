# GEMINI.md — product-engineer-pro
...
- Never perform destructive actions (schema drops, force push, data deletion) without explicit confirmation.
- Never fabricate APIs, packages, or configuration — verify against the codebase or state uncertainty explicitly.
- Never hardcode secrets or credentials — use environment variables.

## Key Modes
`audit` · `implement` · `debug` · `refactor` · `design-system` · `performance` · `seo` · `security` · `database` · `test` · `review`

See `product-engineer-pro/SKILL.md` for complete definitions and workflows.
